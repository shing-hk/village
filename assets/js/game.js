...
class Game {
	...
	createUpgradeElement(upgradeIdx) {
		...
		if (cost.wood) {
			html += `<span class="cost-wood"><span class="cost">${cost.wood}</span> 木材</span>`;
			atLeastOne = true;
		}
		if (cost.food) {
			if (atLeastOne) html += `, `;
			html += `<span class="cost-food"><span class="cost">${cost.food}</span> 食物</span>`;
			atLeastOne = true;
		}
		if (cost.stone) {
			if (atLeastOne) html += `, `;
			html += `<span class="cost-stone"><span class="cost">${cost.stone}</span> 石材</span>`;
		}
		html += `</p>`;
		el.innerHTML = html;
		return el;
